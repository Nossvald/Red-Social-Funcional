...
-- Nuestras funciones fueron definidas en ingles, para distinguirlas de las dadas por la cátedra
...
sacarRepetidos [] = []
sacarRepetidos (x:xs) | elem x (sacarRepetidos xs) = sacarRepetidos xs
...
nombresDeUsuarios (user:users, relations, posts) = sacarRepetidos((nombreDeUsuario user):(nombresDeUsuarios (users,relations,posts)))
...
-- En caso de serlo, devolvemos el usuario. Caso contrario devolvemos el valor regresado por el paso anterior en la recursion.
...
-- El caso base, donde no hay relaciones de amistad en la red social, y el caso recursivo.
-- En el caso recursivo usamos guardas para que sea todo más declarativo. Véase los comentarios en las guardas para más detalles.
existeSecuenciaDeAmigos :: RedSocial -> Usuario -> Usuario -> Bool
existeSecuenciaDeAmigos (_,[],_) _ _ = False
existeSecuenciaDeAmigos (users, relations, posts) user1 user2 | not (hasFriends user1 && hasFriends user2) = False -- Si alguno no tiene amigos, es imposible que exista una cadena.
                                                              | elem user2 (friend:friends) = True -- Si user1 es amigo de user2 la relación es directa, es decir, son amigos.
                                                              | user1 == user2 = True -- Dado los casos anteriores, si llego a este caso es porque user1 tiene amigos, y estamos chequeando si existe una cadena con si mismo user1 -> (cualquier amigo de user1) -> user1.
                                                              | otherwise = theChainIsInTheFirstFriend || theChainIsInTheRestOfFriends -- La relación "puede estar escondida" los otros casos. O bien la cadena (la primera que se encuentre) está en el primer amigo, o está en el resto de ellos. 
                                                                where (friend:friends) = amigosDe (users, relations, posts) user1
                                                                      hasFriends user = amigosDe (users, relations, posts) user /= []
                                                                      theChainIsInTheFirstFriend = existeSecuenciaDeAmigos (users, removeRelations relations user1, posts) friend user2
                                                                      theChainIsInTheRestOfFriends = existeSecuenciaDeAmigos (users, removeRelations relations friend, posts) user1 user2
